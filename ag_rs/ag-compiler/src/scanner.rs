...
impl<'src> Scanner<'src> {
    pub fn new<'s: 'src>(name: String, source: &'s String) -> Self {
        Self {
            file_name: name,
            source,
    ...
    /// Reinitialize the scanner for new source code. This allows for the linker's parser to update itself instead of 
    /// constantly initializing & dropping parsers.
    pub fn re_init<'s: 'src>(&mut self, name: String, source: &'s String) {
        self.file_name = name;
        self.source = source;
    ...
    fn scan_token(&mut self) -> ScanResult {
        ...
        let char = match self.advance() {
            Ok(c) => c,
            Err(_) => {
                if self.has_given_eof {
                    return Err((
                        "Cannot read past the end of a file (COMPILER ERROR! REPORT THIS)"
                            .to_string(),
                        self.line,
                        self.columns.end,
                    ));
                } else {
                    self.has_given_eof = true;
                    return Ok(Token::eof(self.line, self.columns.start..self.columns.end));
                }
            }
        };
    ...
    pub fn scan(&mut self) -> ScanResult {
        ...
        println!(
            "| Debug->Scanner: [{}:{}:{}-{}]: {:?} \"{}\"",
            self.file_name, tok.lexeme, tok.columns.start, tok.columns.end, tok.kind, tok.lexeme
        );
